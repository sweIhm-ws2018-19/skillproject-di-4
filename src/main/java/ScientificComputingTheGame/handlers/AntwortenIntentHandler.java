...
public class AntwortenIntentHandler implements RequestHandler {
	...
	public Optional<Response> handle(HandlerInput input) {
		...
		input.getAttributesManager()
				.setSessionAttributes(Collections.singletonMap(possibleAnswer.getValue(), "Antwort"));

		if (possibleAnswer.getValue() != null) {
			if (possibleAnswer.getValue().toLowerCase()
					.equals(ScientificComputingTheGame.getAntwort().getAntwort().toLowerCase())) {
		...
		return input.getResponseBuilder().withSimpleCard("Antworten", speechText).withSpeech(speechText)
